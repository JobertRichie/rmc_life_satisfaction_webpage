...
cantril_tab <- 
  cantril %>% 
  select(Country, Year, CLS) %>% #selecting relevant variables
  filter(Year %in% c(2011, 2023)) %>% ## Filtering the data for years 2011 & 2023
...
reactable(cantril_tab,
          columns = list(
            abs_change = colDef(name = "Absolute change"),
            rel_change = colDef(name = "Relative change")
          )) %>% 
  add_title(
    title = "Table 1: Self-reported life satisfaction for years 2011 & 2023",
    align = "center",
    font_size = 20
    )
```
...
# Create line chart
cantril %>% 
  filter(Country %in% c("Cameroon", "Malaysia", "Germany", "Cote d'Ivoire", "Morocco", "Brazil",  "Malawi", "Kenya")) %>% 
  ggplot(aes(x = Year,
             y = CLS,
             color = Country)) +
  ...
  labs(y = "Cantril Ladder score",
       title = "Cantril Ladder score evolution over time, from 2011 to 2023",
       subtitle = "Data for Brazil, Cameroon, Cote d'Ivoire, Germany, Kenya, 
Malawi, Malaysia & Morocco")
```
...
function filter_default() {
  var default_countries = ["Cameroon", "Cote d'Ivoire", "Germany", "Morocco", "Malaysia", "Malawi", "Brazil", "Kenya"]; // Set default countries here
